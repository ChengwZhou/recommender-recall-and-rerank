...
def get_user_history_embedding(model, movie_list):
    ...
    movies = MultiMovieClass(movie_list)
    keywords = movies.get_keywords()
    for i in range(len(keywords)):
        movie_vector = np.zeros(32)
        w_num = 0
        for word in keywords[i]:
            w_num += 0
            movie_vector += model.wv[word]
        if not w_num == 0:
            movie_vector = movie_vector / w_num
...
def bulid_recall_movie_feature(df, df_pop, model_wv, top_20_movies, top_view_movies):
    ...
    user_variance = np.sum(np.std(user_history_embedding, axis=0)) / 32
    ...
    df_recall = pd.concat([pd.DataFrame(recall_id), pd.DataFrame(recall_cnt), pd.DataFrame(simil_l),
                           pd.DataFrame([user_variance] * len(recall_cnt)), pd.DataFrame(pop_list)], axis=1)
    return df_recall
...
if __name__ == "__main__":
    ...
    # print(df_recall)
    df_recall.columns = ['userId', 'movieId', 'numFrequency', 'similarityScore', 'userVar', 'moviePopularity']
    label = get_label(df_recall, df_y)
    df_x = df_recall[['numFrequency', 'similarityScore', 'userVar', 'moviePopularity']]
    df_x.to_csv('data/dataset_feature.csv')
    print(df_x.head())